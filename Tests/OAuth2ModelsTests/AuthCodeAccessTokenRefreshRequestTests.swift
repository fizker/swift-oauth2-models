...
final class AuthCodeGrantRefreshTokenRequestTests: XCTestCase {
	var json: [RefreshTokenRequest.CodingKeys: String] = [:]
	...
	func test__setUp__noChanges__decodesCorrectly() throws {
		...
		XCTAssertNoThrow(try decode(data) as RefreshTokenRequest)
	}
	...
	func test__decodable__scopeSetToNull__returnsDecodedObject() throws {
		...
		""".data(using: .utf8)!
		let request = try decode(data) as RefreshTokenRequest
	...
	func test__decodable__invalidGrantType__throws() throws {
		...
		XCTAssertThrowsError(try decode(data) as RefreshTokenRequest)
	}
	...
	func test__encodable__fullObject__encodesAsExpected() throws {
		let object = RefreshTokenRequest(refreshToken: "foo", scope: "bar")
	...
	func test__encodable__minimalObject__encodesAsExpected() throws {
		let object = RefreshTokenRequest(refreshToken: "foo")
		let data = try encode(object)
		let actual = try decode(data) as [String: String]
